...
`.replace(/\s+/g," ").trim();function It({title:t,body:e,narration:n}){const o=typeof n=="string"&&n.trim()?` data-voiceover-script="${Ot(n.trim())}"`:"";return`
  ...
  `,narration:"Next, let's set up your hands. Throughout the study you will respond with the two horizontal arrow keys on your keyboard, the left arrow and the right arrow. The demonstrations will label them No or Yes, and Not on-pattern or On-pattern. How you place your hands matters. Use both hands, and only your index fingers. Place one index finger on each of the two horizontal arrow keys, one finger per hand. Keep them resting on those two keys throughout the study. Do not use two fingers of the same hand — one index finger per hand, on its own response key. When both index fingers are resting on the two response keys, click the button below to continue."});function Co(){const t=b({type:O,stimulus:No,choices:["I am ready"],button_html:n=>`<button class="jspsych-btn" style="margin-top: 1rem;">${n}</button>`,data:{phase:"viewing_distance"}},"viewing_distance"),e=b({type:O,stimulus:$o,choices:["I am ready"],button_html:n=>`<button class="jspsych-btn" style="margin-top: 1rem;">${n}</button>`,data:{phase:"hand_placement"}},"hand_placement");return{timeline:[t,e]}}function Pt(){const t="speaker_volume_instruction",e={type:O,stimulus:`
    ...
    `,choices:["I'm comfortable — continue"],button_html:l=>`<button class="jspsych-btn" id="tac-yes-btn">${l}</button>`,on_load:async()=>{e=!1,n=!1,o=!1,r=0,a=Z;const l=Promise.resolve();We(Z/100);const u=document.getElementById("tac-slider"),s=document.getElementById("tac-slider-readout");u&&u.addEventListener("input",()=>{const y=Number(u.value);a=y,s&&(s.textContent=`${y}%`),We(y/100)});const d=document.getElementById("tac-yes-btn");if(d&&d.addEventListener("click",()=>{n||(o=!0),e=!0},{capture:!0}),await l,e)return;const c=fn(),h=mt(),m=g.tones.duration_ms/1e3,_=(g.tones.duration_ms+Yo)/1e3,f=Ho/1e3,w=[];for(let y=0;y<h.length;y++)w.push(document.getElementById(`tac-box-${y}`));let x=c.currentTime+Fo/1e3;try{for(let y=0;y<Go&&!e;y++){r=y+1;for(let A=0;A<h.length&&!e;A++){const D=x;if(yn(A,{when:D}),await Te(c,D),e)break;const K=w[A];K&&K.classList.add("playing"),await Te(c,D+m),K&&K.classList.remove("playing"),x=D+_}e||(await Te(c,x+f),x+=f)}}catch{}if(!o&&!e){n=!0;const y=document.getElementById("tac-yes-btn");y&&y.click()}},on_finish:l=>{pt();const u=$e();u.current&&!u.current._state?.finished&&u.current.cancel(),l.tones_audible=o&&!n,l.user_playback_pct=a,l.user_playback_gain=a/100,l.sweeps_completed=r},data:{phase:"tone_audibility_and_comfort"}};return v(t,At),b(i,t)}function Ko(){return{timeline:[{type:O,stimulus:`
  ...
  `}function Zt(){const t=g.blocks.practice_trials_per_type,e=t*5,n=Math.round(g.practice.accuracy_criterion*100),o={type:N,stimulus:`
    ...
    `,choices:[" "],data:{phase:"main_a_intro"},on_load:()=>{ft()}};return v(t,rn),b(e,t)}function an(){const t="main_b_intro",e={type:N,stimulus:`
      <div style="max-width: 36rem; margin: 0 auto;">
    ...
    `});
//# sourceMappingURL=main-Bgswh-yw.js.map

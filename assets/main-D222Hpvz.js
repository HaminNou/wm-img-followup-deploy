...
`.replace(/\s+/g," ").trim();function It({title:t,body:e,narration:n}){const o=typeof n=="string"&&n.trim()?` data-voiceover-script="${Ot(n.trim())}"`:"";return`
  ...
  `}const ze=50,j=ze/2,St=ze*Math.SQRT2/2+1,Le=St-j+.5,de=ze+2*Le,Ze={cx:35,cy:15},et={cx:15,cy:35},wo=2.5,_o=2.5,tt=100,vo=30,ko=12,To=12,xo=[1,2,3,7,10,11,12,13,14,17,21,22,23],ue=10,Eo="#FFFFFF",Io=.33;function So(t,e,n){const r=n*.382,a=[];for(let i=0;i<10;i++){const l=i%2===0?n:r,u=-Math.PI/2+i*Math.PI/5;a.push(`${t+l*Math.cos(u)},${e+l*Math.sin(u)}`)}return a.join(" ")}const Oo=So(et.cx,et.cy,_o),Po="rgba(0, 0, 0, 0.229)",jo="rgba(0, 0, 0, 0.55)",nt="#000",ot="rgba(0, 0, 0, 0.85)";function Lo(t,{arrowAngleDeg:e=vo}={}){const n=new Set(t),o=n.has("tone")||n.has("qmark"),r=n.has("pattern"),a=`<circle cx="${j}" cy="${j}" r="${St}" fill="none" stroke="${Po}" stroke-width="0.5" />`,i=o||r?"":`<circle cx="${j}" cy="${j}" r="0.65" fill="${jo}" />`,l=r?xo.map(f=>{const w=f%5,x=Math.floor(f/5),g=w*ue,N=x*ue;return`<rect x="${g}" y="${N}" width="${ue}" height="${ue}" fill="${Eo}" opacity="${Io}" />`}).join(""):"",u=n.has("dot")?`<circle cx="${Ze.cx}" cy="${Ze.cy}" r="${wo}" fill="${nt}" />`:"",s=n.has("star")?`<polygon points="${Oo}" fill="${nt}" />`:"",d=n.has("tone")?`<text x="${j}" y="${j}" text-anchor="middle" dominant-baseline="central" fill="${ot}" font-size="${ko}" font-family="Arial, sans-serif" font-weight="bold">♪</text>`:"",c=n.has("qmark")?`<text x="${j}" y="${j}" text-anchor="middle" dominant-baseline="central" fill="${ot}" font-size="${To}" font-family="Arial, sans-serif" font-weight="bold">?</text>`:"",h=`
  ...
  `}function No(){return"./assets/rotation-arrow.png"}function k({title:t,body:e,narration:n,exemplar:o=[],arrowAngleDeg:r}){const a=typeof n=="string"&&n.trim()?` data-voiceover-script="${Ot(n.trim())}"`:"",i=Lo(o,typeof r=="number"?{arrowAngleDeg:r}:{});return`
  ...
  `}function Ot(t){return t.replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}const Ao=It({title:"Sit comfortably for the study",body:`
  ...
  `,narration:"Next, let's set up your hands. Throughout the study you will respond with the two horizontal arrow keys on your keyboard, the left arrow and the right arrow. The demonstrations will label them No or Yes, and Not on-pattern or On-pattern. How you place your hands matters. Use both hands, and only your index fingers. Place one index finger on each of the two horizontal arrow keys, one finger per hand. Keep them resting on those two keys throughout the study. Do not use two fingers of the same hand — one index finger per hand, on its own response key. When both index fingers are resting on the two response keys, click the button below to continue."});function Co(){const t=b({type:O,stimulus:Ao,choices:["I am ready"],button_html:n=>`<button class="jspsych-btn" style="margin-top: 1rem;">${n}</button>`,data:{phase:"viewing_distance"}},"viewing_distance"),e=b({type:O,stimulus:$o,choices:["I am ready"],button_html:n=>`<button class="jspsych-btn" style="margin-top: 1rem;">${n}</button>`,data:{phase:"hand_placement"}},"hand_placement");return{timeline:[t,e]}}function Pt(){const t="speaker_volume_instruction",e={type:O,stimulus:`
    ...
    `,choices:["I'm comfortable — continue"],button_html:l=>`<button class="jspsych-btn" id="tac-yes-btn">${l}</button>`,on_load:async()=>{e=!1,n=!1,o=!1,r=0,a=Z;const l=Promise.resolve();Ge(Z/100);const u=document.getElementById("tac-slider"),s=document.getElementById("tac-slider-readout");u&&u.addEventListener("input",()=>{const g=Number(u.value);a=g,s&&(s.textContent=`${g}%`),Ge(g/100)});const d=document.getElementById("tac-yes-btn");if(d&&d.addEventListener("click",()=>{n||(o=!0),e=!0},{capture:!0}),await l,e)return;const c=mn(),h=mt(),m=y.tones.duration_ms/1e3,_=(y.tones.duration_ms+Bo)/1e3,f=Ho/1e3,w=[];for(let g=0;g<h.length;g++)w.push(document.getElementById(`tac-box-${g}`));let x=c.currentTime+Fo/1e3;try{for(let g=0;g<Go&&!e;g++){r=g+1;for(let N=0;N<h.length&&!e;N++){const D=x;if(fn(N,{when:D}),await ke(c,D),e)break;const K=w[N];K&&K.classList.add("playing"),await ke(c,D+m),K&&K.classList.remove("playing"),x=D+_}e||(await ke(c,x+f),x+=f)}}catch{}if(!o&&!e){n=!0;const g=document.getElementById("tac-yes-btn");g&&g.click()}},on_finish:l=>{ht();const u=pt();u.current&&!u.current._state?.finished&&u.current.cancel(),l.tones_audible=o&&!n,l.user_playback_pct=a,l.user_playback_gain=a/100,l.sweeps_completed=r},data:{phase:"tone_audibility_and_comfort"}};return v(t,Nt),b(i,t)}function Ko(){return{timeline:[{type:O,stimulus:`
      ...
      `,narration:"Very important: keep the picture of the rotated pattern in mind during the entire trial. You may drop it only when the central question mark appears — that is when the dot tests begin.",exemplar:["pattern"]})],a={type:re,pages:r,...Bt,data:{phase:"instructions_dual_i_d"}};return W(a.data.phase,Ht),U(a)}function Ft(){const t=y.memory.set_size,e=y.imagery.stars_per_trial,n=t+e,o="Task 5 — Tones and pattern together",r=[k({title:o,body:`
      ...
      `,narration:`After the ${n} things, we test your tone memory ${t} times. Each test plays a tone. Press Yes, the right arrow, if you heard this tone before. Press No, the left arrow, if you did not. You have 3 seconds for each star and each tone test. If you answer in time, the star or central question mark turns green for correct or red for incorrect.`,exemplar:["qmark"]}),k({title:o,body:`
        ...
        <p>Please make sure your audio volume is still at the level you
        set earlier.</p>
      `,narration:"Very important: keep the picture of the rotated pattern in mind during the entire trial. You may drop it only when the central question mark appears — that is when the tone tests begin. Please make sure your audio volume is still at the level you set earlier.",exemplar:["pattern"]})],a={type:re,pages:r,...Bt,data:{phase:"instructions_dual_i_t"}};return W(a.data.phase,Ft),U(a)}const nr=4,Gt=1e3*nr,or=2e3,rr=400,E=500,T=400,q=1200,ar=700,ir=450,sr=220,Y=1500,qe=1e3,lr=Object.freeze([-90,-75,-60,-45,-30,-15,15,30,45,60,75,90]);function Ye(){return Ne().pick(lr)}const cr="./assets/rotation-arrow.png";function dr(t,e){return`
    <style>
  ...
  `}function Zt(){const t=y.blocks.practice_trials_per_type,e=t*5,n=Math.round(y.practice.accuracy_criterion*100),o={type:A,stimulus:`
    ...
    `,button_label:"Continue",on_finish:o=>{o.phase="music_experience",o.music_experience_responses=J.reduce((r,a)=>{const i=o.response?.[a.id];return r[a.id]=i===""||i===null||i===void 0?null:String(i),r},{})}};return v(e,nn),b(n,e)}function $r(){return[tn(),nn()]}function on(){const t="questionnaire_intro",e={type:A,stimulus:`
      <div style="max-width: 38rem; margin: 0 auto; text-align: left;">
    ...
    `,choices:[" "],data:{phase:"questionnaire_intro"}};return v(t,on),b(e,t)}function Cr(){return[on(),...Ar(),...$r()]}function rn(){const t="main_a_intro",e={type:A,stimulus:`
      <div style="max-width: 36rem; margin: 0 auto;">
    ...
    `,choices:[" "],data:{phase:"main_a_intro"},on_load:()=>{ft()}};return v(t,rn),b(e,t)}function an(){const t="main_b_intro",e={type:A,stimulus:`
      <div style="max-width: 36rem; margin: 0 auto;">
    ...
    `,choices:[" "],data:{phase:"end_of_task_intro"},on_load:()=>{wn(),ft()}};return v(t,sn),b(e,t)}const S=dn(),Rr=un();function Dr(){if(mt(),typeof Image<"u"){const t=new Image;t.src="./assets/rotation-arrow.png"}}Dr();const _e=hn({display_element:void 0,on_trial_start:Mn(),on_data_update:t=>{dt(t).catch(e=>{console.error("[runner] storage append failed",e),ut(e)})},on_finish:()=>{const t=_e.data?.get?.();t&&typeof t.addProperties=="function"&&t.addProperties({session_id:S.params.session_id,prolific_pid:S.params.prolific_pid,study_id:S.params.study_id,is_preview:S.params.is_preview,is_pilot_run:S.is_pilot_run,pilot_label:S.pilot_label,prng_seed_uint32:S.prng_seed_uint32,schema_version:S.schema_version,build_id:S.build_id}),console.info("[runner] session",S,"visibility",qo())}});window.jsPsych=_e;zn(_e);const ct=_n(),Mr=[...Vo(),...Pr(),rn(),...ct.mainA.nodes,vn(),an(),...ct.mainB.nodes,sn(),...Cr(),kn({session:S}),Tn()];Rr.then(()=>_e.run(Mr)).catch(t=>{console.error("[runner] fatal startup failure",t);const e=document.body;e.innerHTML=`
    ...
    `});
//# sourceMappingURL=main-D222Hpvz.js.map

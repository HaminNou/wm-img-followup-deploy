...
`.replace(/\s+/g," ").trim();function It({title:t,body:e,narration:n}){const o=typeof n=="string"&&n.trim()?` data-voiceover-script="${Ot(n.trim())}"`:"";return`
  ...
  `}const Be=50,j=Be/2,St=Be*Math.SQRT2/2+1,Ne=St-j+.5,ce=Be+2*Ne,et={cx:35,cy:15},tt={cx:15,cy:35},_o=2.5,vo=2.5,nt=100,ko=30,To=12,xo=12,Eo=[1,2,3,7,10,11,12,13,14,17,21,22,23],de=10,Io="#FFFFFF",So=.33;function Oo(t,e,n){const r=n*.382,a=[];for(let i=0;i<10;i++){const l=i%2===0?n:r,u=-Math.PI/2+i*Math.PI/5;a.push(`${t+l*Math.cos(u)},${e+l*Math.sin(u)}`)}return a.join(" ")}const Po=Oo(tt.cx,tt.cy,vo),jo="rgba(0, 0, 0, 0.229)",Lo="rgba(0, 0, 0, 0.55)",ot="#000",rt="rgba(0, 0, 0, 0.85)";function No(t,{arrowAngleDeg:e=ko}={}){const n=new Set(t),o=n.has("tone")||n.has("qmark"),r=n.has("pattern"),a=`<circle cx="${j}" cy="${j}" r="${St}" fill="none" stroke="${jo}" stroke-width="0.5" />`,i=o||r?"":`<circle cx="${j}" cy="${j}" r="0.65" fill="${Lo}" />`,l=r?Eo.map(f=>{const w=f%5,x=Math.floor(f/5),g=w*de,N=x*de;return`<rect x="${g}" y="${N}" width="${de}" height="${de}" fill="${Io}" opacity="${So}" />`}).join(""):"",u=n.has("dot")?`<circle cx="${et.cx}" cy="${et.cy}" r="${_o}" fill="${ot}" />`:"",s=n.has("star")?`<polygon points="${Po}" fill="${ot}" />`:"",d=n.has("tone")?`<text x="${j}" y="${j}" text-anchor="middle" dominant-baseline="central" fill="${rt}" font-size="${To}" font-family="Arial, sans-serif" font-weight="bold">♪</text>`:"",c=n.has("qmark")?`<text x="${j}" y="${j}" text-anchor="middle" dominant-baseline="central" fill="${rt}" font-size="${xo}" font-family="Arial, sans-serif" font-weight="bold">?</text>`:"",h=`
  ...
  `,narration:"Next, let's set up your hands. Throughout the study you will respond with the two horizontal arrow keys on your keyboard, the left arrow and the right arrow. The demonstrations will label them No or Yes, and Not on-pattern or On-pattern. How you place your hands matters. Use both hands, and only your index fingers. Place one index finger on each of the two horizontal arrow keys, one finger per hand. Keep them resting on those two keys throughout the study. Do not use two fingers of the same hand — one index finger per hand, on its own response key. When both index fingers are resting on the two response keys, click the button below to continue."});function Ro(){const t=b({type:O,stimulus:$o,choices:["I am ready"],button_html:n=>`<button class="jspsych-btn" style="margin-top: 1rem;">${n}</button>`,data:{phase:"viewing_distance"}},"viewing_distance"),e=b({type:O,stimulus:Co,choices:["I am ready"],button_html:n=>`<button class="jspsych-btn" style="margin-top: 1rem;">${n}</button>`,data:{phase:"hand_placement"}},"hand_placement");return{timeline:[t,e]}}function Pt(){const t="speaker_volume_instruction",e={type:O,stimulus:`
    ...
    `,choices:["I'm comfortable — continue"],button_html:l=>`<button class="jspsych-btn" id="tac-yes-btn">${l}</button>`,on_load:async()=>{e=!1,n=!1,o=!1,r=0,a=Z;const l=Promise.resolve();We(Z/100);const u=document.getElementById("tac-slider"),s=document.getElementById("tac-slider-readout");u&&u.addEventListener("input",()=>{const g=Number(u.value);a=g,s&&(s.textContent=`${g}%`),We(g/100)});const d=document.getElementById("tac-yes-btn");if(d&&d.addEventListener("click",()=>{n||(o=!0),e=!0},{capture:!0}),await l,e)return;const c=fn(),h=mt(),m=y.tones.duration_ms/1e3,_=(y.tones.duration_ms+Ho)/1e3,f=Fo/1e3,w=[];for(let g=0;g<h.length;g++)w.push(document.getElementById(`tac-box-${g}`));let x=c.currentTime+Go/1e3;try{for(let g=0;g<Wo&&!e;g++){r=g+1;for(let N=0;N<h.length&&!e;N++){const D=x;if(gn(N,{when:D}),await Te(c,D),e)break;const K=w[N];K&&K.classList.add("playing"),await Te(c,D+m),K&&K.classList.remove("playing"),x=D+_}e||(await Te(c,x+f),x+=f)}}catch{}if(!o&&!e){n=!0;const g=document.getElementById("tac-yes-btn");g&&g.click()}},on_finish:l=>{pt();const u=$e();u.current&&!u.current._state?.finished&&u.current.cancel(),l.tones_audible=o&&!n,l.user_playback_pct=a,l.user_playback_gain=a/100,l.sweeps_completed=r},data:{phase:"tone_audibility_and_comfort"}};return v(t,Nt),b(i,t)}function Vo(){return{timeline:[{type:O,stimulus:`
      ...
      `,narration:`After the ${n} things, we test your tone memory ${t} times. Each test plays a tone. Press Yes, the right arrow, if you heard this tone before. Press No, the left arrow, if you did not. You have 3 seconds for each star and each tone test. If you answer in time, the star or central question mark turns green for correct or red for incorrect.`,exemplar:["qmark"]}),k({title:o,body:`
        ...
        <p>Please make sure your audio volume is still at the level you
        set earlier.</p>
      `,narration:"Very important: keep the picture of the rotated pattern in mind during the entire trial. You may drop it only when the central question mark appears — that is when the tone tests begin. Please make sure your audio volume is still at the level you set earlier.",exemplar:["pattern"]})],a={type:re,pages:r,...Yt,data:{phase:"instructions_dual_i_t"}};return W(a.data.phase,Ft),U(a)}const or=4,Gt=1e3*or,rr=2e3,ar=400,E=500,T=400,q=1200,ir=700,sr=450,lr=220,B=1500,Ye=1e3,cr=Object.freeze([-90,-75,-60,-45,-30,-15,15,30,45,60,75,90]);function He(){return Ae().pick(cr)}const dr="./assets/rotation-arrow.png";function ur(t,e){return`
    <style>
  ...
  `}function Zt(){const t=y.blocks.practice_trials_per_type,e=t*5,n=Math.round(y.practice.accuracy_criterion*100),o={type:A,stimulus:`
    ...
    `,button_label:"Continue",on_finish:o=>{o.phase="music_experience",o.music_experience_responses=J.reduce((r,a)=>{const i=o.response?.[a.id];return r[a.id]=i===""||i===null||i===void 0?null:String(i),r},{})}};return v(e,nn),b(n,e)}function Cr(){return[tn(),nn()]}function on(){const t="questionnaire_intro",e={type:A,stimulus:`
      <div style="max-width: 38rem; margin: 0 auto; text-align: left;">
    ...
    `,choices:[" "],data:{phase:"questionnaire_intro"}};return v(t,on),b(e,t)}function Rr(){return[on(),...$r(),...Cr()]}function rn(){const t="main_a_intro",e={type:A,stimulus:`
      <div style="max-width: 36rem; margin: 0 auto;">
    ...
    `,choices:[" "],data:{phase:"end_of_task_intro"},on_load:()=>{_n(),ft()}};return v(t,sn),b(e,t)}const S=un(),Dr=hn();function Mr(){if(mt(),typeof Image<"u"){const t=new Image;t.src="./assets/rotation-arrow.png"}}Mr();const we=pn({display_element:void 0,on_trial_start:zn(),on_data_update:t=>{ut(t).catch(e=>{console.error("[runner] storage append failed",e),ht(e)})},on_finish:()=>{const t=we.data?.get?.();t&&typeof t.addProperties=="function"&&t.addProperties({session_id:S.params.session_id,prolific_pid:S.params.prolific_pid,study_id:S.params.study_id,is_preview:S.params.is_preview,is_pilot_run:S.is_pilot_run,pilot_label:S.pilot_label,prng_seed_uint32:S.prng_seed_uint32,schema_version:S.schema_version,build_id:S.build_id}),console.info("[runner] session",S,"visibility",Bo())}});window.jsPsych=we;qn(we);const dt=vn(),zr=[...Jo(),...jr(),rn(),...dt.mainA.nodes,kn(),an(),...dt.mainB.nodes,sn(),...Rr(),Tn({session:S}),xn()];Dr.then(()=>we.run(zr)).catch(t=>{console.error("[runner] fatal startup failure",t);const e=document.body;e.innerHTML=`
    ...
    `});
//# sourceMappingURL=main-COfC4ZIo.js.map

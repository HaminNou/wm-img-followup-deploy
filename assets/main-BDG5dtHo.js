...
`.replace(/\s+/g," ").trim();function wt({title:t,body:e,narration:n}){const r=typeof n=="string"&&n.trim()?` data-voiceover-script="${vt(n.trim())}"`:"";return`
  ...
  `}const Le=50,j=Le/2,_t=Le*Math.SQRT2/2+1,Ee=_t-j+.5,ie=Le+2*Ee,He={cx:35,cy:15},Ge={cx:15,cy:35},lr=2.5,cr=2.5,Ue=100,ur=30,dr=12,hr=12,pr=[1,2,3,7,10,11,12,13,14,17,21,22,23],se=10,mr="#FFFFFF",fr=.33;function yr(t,e,n){const o=n*.382,a=[];for(let i=0;i<10;i++){const s=i%2===0?n:o,d=-Math.PI/2+i*Math.PI/5;a.push(`${t+s*Math.cos(d)},${e+s*Math.sin(d)}`)}return a.join(" ")}const gr=yr(Ge.cx,Ge.cy,cr),br="rgba(0, 0, 0, 0.229)",wr="rgba(0, 0, 0, 0.55)",We="#000",Ve="rgba(0, 0, 0, 0.85)";function _r(t,{arrowAngleDeg:e=ur}={}){const n=new Set(t),r=n.has("tone")||n.has("qmark"),o=n.has("pattern"),a=`<circle cx="${j}" cy="${j}" r="${_t}" fill="none" stroke="${br}" stroke-width="0.5" />`,i=r||o?"":`<circle cx="${j}" cy="${j}" r="0.65" fill="${wr}" />`,s=o?pr.map(y=>{const k=y%5,x=Math.floor(y/5),f=k*se,L=x*se;return`<rect x="${f}" y="${L}" width="${se}" height="${se}" fill="${mr}" opacity="${fr}" />`}).join(""):"",d=n.has("dot")?`<circle cx="${He.cx}" cy="${He.cy}" r="${lr}" fill="${We}" />`:"",l=n.has("star")?`<polygon points="${gr}" fill="${We}" />`:"",u=n.has("tone")?`<text x="${j}" y="${j}" text-anchor="middle" dominant-baseline="central" fill="${Ve}" font-size="${dr}" font-family="Arial, sans-serif" font-weight="bold">♪</text>`:"",c=n.has("qmark")?`<text x="${j}" y="${j}" text-anchor="middle" dominant-baseline="central" fill="${Ve}" font-size="${hr}" font-family="Arial, sans-serif" font-weight="bold">?</text>`:"",h=`
  ...
  `,narration:"Next, let's set up your hands. Throughout the study you will respond with the two horizontal arrow keys on your keyboard, the left arrow and the right arrow. The demonstrations will label them No or Yes, and Not on-pattern or On-pattern. How you place your hands matters. Use both hands, and only your index fingers. Place one index finger on each of the two horizontal arrow keys, one finger per hand. Keep them resting on those two keys throughout the study. Do not use two fingers of the same hand — one index finger per hand, on its own response key. When both index fingers are resting on the two response keys, click the button below to continue."});function xr(){const t=b({type:S,stimulus:kr,choices:["I am ready"],button_html:n=>`<button class="jspsych-btn" style="margin-top: 1rem;">${n}</button>`,data:{phase:"viewing_distance"}},"viewing_distance"),e=b({type:S,stimulus:Tr,choices:["I am ready"],button_html:n=>`<button class="jspsych-btn" style="margin-top: 1rem;">${n}</button>`,data:{phase:"hand_placement"}},"hand_placement");return{timeline:[t,e]}}function kt(){const t="speaker_volume_instruction",e={type:S,stimulus:`
    ...
    `,choices:["I'm comfortable — continue"],button_html:s=>`<button class="jspsych-btn" id="tac-yes-btn">${s}</button>`,on_load:async()=>{e=!1,n=!1,r=!1,o=0,a=J;const s=Promise.resolve();qe(J/100);const d=document.getElementById("tac-slider"),l=document.getElementById("tac-slider-readout");d&&d.addEventListener("input",()=>{const f=Number(d.value);a=f,l&&(l.textContent=`${f}%`),qe(f/100)});const u=document.getElementById("tac-yes-btn");if(u&&u.addEventListener("click",()=>{n||(r=!0),e=!0},{capture:!0}),await s,e)return;const c=sn(),h=lt(),m=g.tones.duration_ms/1e3,w=(g.tones.duration_ms+Lr)/1e3,y=Nr/1e3,k=[];for(let f=0;f<h.length;f++)k.push(document.getElementById(`tac-box-${f}`));let x=c.currentTime+Ar/1e3;try{for(let f=0;f<$r&&!e;f++){o=f+1;for(let L=0;L<h.length&&!e;L++){const R=x;if(ln(L,{when:R}),await ge(c,R),e)break;const G=k[L];G&&G.classList.add("playing"),await ge(c,R+m),G&&G.classList.remove("playing"),x=R+w}e||(await ge(c,x+y),x+=y)}}catch{}if(!r&&!e){n=!0;const f=document.getElementById("tac-yes-btn");f&&f.click()}},on_finish:s=>{it();const d=st();d.current&&!d.current._state?.finished&&d.current.cancel(),s.tones_audible=r&&!n,s.user_playback_pct=a,s.user_playback_gain=a/100,s.sweeps_completed=o},data:{phase:"tone_audibility_and_comfort"}};return v(t,Et),b(i,t)}function qr(){return{timeline:[{type:S,stimulus:`
        ...
        `,choices:["Close"],data:{phase:"tone_audibility_fail"},on_finish:()=>{typeof window<"u"&&window.jsPsych?.abortExperiment&&window.jsPsych.abortExperiment("Tone audibility failed.")}}],conditional_function:()=>{const t=window.jsPsych?.data?.get().filter({phase:"tone_audibility_and_comfort"}).last(1).values()[0];return t?t.tones_audible===!1:!1}}}function zr(){return[Fn(),Hn(),ht(),Cn(),pt(),xr(),Zn(),kt(),xt(),jr(),Et(),qr()]}var Mr="2.1.0";const Dr={name:"instructions",version:Mr,parameters:{pages:{type:p.HTML_STRING,default:void 0,array:!0},key_forward:{type:p.KEY,default:"ArrowRight"},key_backward:{type:p.KEY,default:"ArrowLeft"},allow_backward:{type:p.BOOL,default:!0},allow_keys:{type:p.BOOL,default:!0},show_clickable_nav:{type:p.BOOL,default:!1},show_page_number:{type:p.BOOL,default:!1},page_label:{type:p.STRING,default:"Page"},button_label_previous:{type:p.STRING,default:"Previous"},button_label_next:{type:p.STRING,default:"Next"},on_page_change:{type:p.FUNCTION,pretty_name:"Page change callback",default:function(t){}}},data:{view_history:{type:p.COMPLEX,array:!0,nested:{page_index:{type:p.INT},viewing_time:{type:p.INT}}},rt:{type:p.INT}},citations:{apa:"de Leeuw, J. R., Gilbert, R. A., & Luchterhandt, B. (2023). jsPsych: Enabling an Open-Source Collaborative Ecosystem of Behavioral Experiments. Journal of Open Source Software, 8(85), 5351. https://doi.org/10.21105/joss.05351 ",bibtex:'@article{Leeuw2023jsPsych, 	author = {de Leeuw, Joshua R. and Gilbert, Rebecca A. and Luchterhandt, Bj{\\" o}rn}, 	journal = {Journal of Open Source Software}, 	doi = {10.21105/joss.05351}, 	issn = {2475-9066}, 	number = {85}, 	year = {2023}, 	month = {may 11}, 	pages = {5351}, 	publisher = {Open Journals}, 	title = {jsPsych: Enabling an {Open}-{Source} {Collaborative} {Ecosystem} of {Behavioral} {Experiments}}, 	url = {https://joss.theoj.org/papers/10.21105/joss.05351}, 	volume = {8}, }  '}};class ee{constructor(e){this.jsPsych=e}static{this.info=Dr}trial(e,n){var r=0,o=[],a=performance.now(),i=a;function s(){this.id==="jspsych-instructions-back"?u():this.id==="jspsych-instructions-next"&&l()}function d(){var y=n.pages[r],k="";if(n.show_page_number&&(k="<span style='margin: 0 1em;' class='jspsych-instructions-pagenum'>"+n.page_label+" "+(r+1)+"/"+n.pages.length+"</span>"),n.show_clickable_nav){var x="<div class='jspsych-instructions-nav' style='padding: 10px 0px;'>";if(n.allow_backward){var f=r>0?"":"disabled='disabled'";x+="<button id='jspsych-instructions-back' class='jspsych-btn' style='margin-right: 5px;' "+f+">&lt; "+n.button_label_previous+"</button>"}n.pages.length>1&&n.show_page_number&&(x+=k),x+="<button id='jspsych-instructions-next' class='jspsych-btn'style='margin-left: 5px;'>"+n.button_label_next+" &gt;</button></div>",y+=x,e.innerHTML=y,r!=0&&n.allow_backward&&e.querySelector("#jspsych-instructions-back").addEventListener("click",s,{once:!0}),e.querySelector("#jspsych-instructions-next").addEventListener("click",s,{once:!0})}else n.show_page_number&&n.pages.length>1&&(y+="<div class='jspsych-instructions-pagenum'>"+k+"</div>"),e.innerHTML=y}function l(){c(),r++,r>=n.pages.length?h():d(),n.on_page_change(r)}function u(){c(),r--,d(),n.on_page_change(r)}function c(){var y=performance.now(),k=Math.round(y-i);o.push({page_index:r,viewing_time:k}),i=y}const h=()=>{n.allow_keys&&this.jsPsych.pluginAPI.cancelKeyboardResponse(w);var y={view_history:o,rt:Math.round(performance.now()-a)};this.jsPsych.finishTrial(y)},m=y=>{w=this.jsPsych.pluginAPI.getKeyboardResponse({callback_function:m,valid_responses:[n.key_forward,n.key_backward],rt_method:"performance",persist:!1,allow_held_key:!1}),this.jsPsych.pluginAPI.compareKeys(y.key,n.key_backward)&&r!==0&&n.allow_backward&&u(),this.jsPsych.pluginAPI.compareKeys(y.key,n.key_forward)&&l()};if(d(),n.allow_keys)var w=this.jsPsych.pluginAPI.getKeyboardResponse({callback_function:m,valid_responses:[n.key_forward,n.key_backward],rt_method:"performance",persist:!1})}simulate(e,n,r,o){n=="data-only"&&(o(),this.simulate_data_only(e,r)),n=="visual"&&this.simulate_visual(e,r,o)}create_simulation_data(e,n){let r=0,o=0,a=[];if(!n.data?.view_history&&!n.data?.rt)for(;r!==e.pages.length;){const d=Math.round(this.jsPsych.randomization.sampleExGaussian(3e3,300,.0033333333333333335));a.push({page_index:r,viewing_time:d}),o+=d,r==0||!e.allow_backward||this.jsPsych.randomization.sampleBernoulli(.9)==1?r++:r--}if(!n.data?.view_history&&n.data?.rt){for(o=n.data.rt;r!==e.pages.length;)a.push({page_index:r,viewing_time:null}),r==0||!e.allow_backward||this.jsPsych.randomization.sampleBernoulli(.9)==1?r++:r--;const d=n.data.rt/a.length;let l=0;for(const h of a){const m=Math.round(this.jsPsych.randomization.sampleExGaussian(d,d/10,1/(d/10)));h.viewing_time=m,l+=m}const u=n.data.rt-l,c=Math.round(u/a.length);for(const h of a)h.viewing_time+=c}if(n.data?.view_history&&!n.data?.rt){a=n.data.view_history,o=0;for(const d of n.data.view_history)o+=d.viewing_time}const i={view_history:a,rt:o},s=this.jsPsych.pluginAPI.mergeSimulationData(i,n);return this.jsPsych.pluginAPI.ensureSimulationDataConsistency(e,s),s}simulate_data_only(e,n){const r=this.create_simulation_data(e,n);this.jsPsych.finishTrial(r)}simulate_visual(e,n,r){const o=this.create_simulation_data(e,n),a=this.jsPsych.getDisplayElement();this.trial(a,e),r();const i=u=>{e.allow_keys?this.jsPsych.pluginAPI.pressKey(e.key_forward,u):e.show_clickable_nav&&this.jsPsych.pluginAPI.clickTarget(a.querySelector("#jspsych-instructions-next"),u)},s=u=>{e.allow_keys?this.jsPsych.pluginAPI.pressKey(e.key_backward,u):e.show_clickable_nav&&this.jsPsych.pluginAPI.clickTarget(a.querySelector("#jspsych-instructions-back"),u)};let d=0,l=0;for(let u=0;u<o.view_history.length;u++)u==o.view_history.length-1?i(l+o.view_history[u].viewing_time):(o.view_history[u+1].page_index>d&&i(l+o.view_history[u].viewing_time),o.view_history[u+1].page_index<d&&s(l+o.view_history[u].viewing_time),l+=o.view_history[u].viewing_time,d=o.view_history[u+1].page_index)}}const It='<span style="color: #0DE72E;"><strong>green</strong></span>',Ot='<span style="color: #FF1818;"><strong>red</strong></span>',Pt=Object.freeze({key_forward:" ",key_backward:"ArrowLeft",allow_backward:!0,allow_keys:!0,show_clickable_nav:!0,show_page_number:!0,page_label:"Page",button_label_previous:"Previous",button_label_next:"Next"});function St(){const t=g.memory.set_size,e="Task 1 — Remember the dots",n=[_({title:e,body:`
  ...
  `}function Ut(){const t=g.blocks.practice_trials_per_type,e=t*5,n=Math.round(g.practice.accuracy_criterion*100),r={type:N,stimulus:`
    ...
    `});
//# sourceMappingURL=main-BDG5dtHo.js.map
